...
std::pair<MatrixXd, VectorXd> stamp(std::list<Element> &elements, int numNodes,
  ...
  for (auto &element : elements) {
    ...
    if (element.getName()[0] == 'R') {
      ...
      positiveNode = element["positiveNode"];
      negativeNode = element["negativeNode"];
  ...
  return std::make_pair(
      A(Eigen::indexing::lastN(size - 1), Eigen::indexing::lastN(size - 1)),
      rhs(Eigen::indexing::lastN(size - 1)));
}
