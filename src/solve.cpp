...
double i_emitter(double v_be, double v_bc) {
  return -1 * I_SAT_EMITTER * (exp(v_be / V_T_EMITTER) - 1) +
         ALPHA_R * I_SAT_COLLECTOR * (exp(v_bc / V_T_COLLECTOR) - 1);
}

double i_collector(double v_be, double v_bc) {
  return ALPHA_F * I_SAT_EMITTER * (exp(v_be / V_T_EMITTER) - 1) -
         I_SAT_COLLECTOR * (exp(v_bc / V_T_COLLECTOR) - 1);
}
...
double g_ec(double v_b, double v_c) {
  return ALPHA_R * I_SAT_COLLECTOR / V_T_COLLECTOR *
         exp((v_b - v_c) / V_T_COLLECTOR);
}
...
double I_eq_diode(double id, double G_eq, double v_positive,
                  double v_negative) {
...
double i_source(double v_gs, double v_ds) { return -1 * i_drain(v_gs, v_ds); }
...
Hg::Hg(Element element, int size) {
  ...
  m_size = size;
  if (m_elementType == "D") {
    m_negativeNode = element["negativeNode"];
    m_positiveNode = element["positiveNode"];
  } else if (m_elementType == "Q") {
    m_base = element["base"];
    m_collector = element["collector"];
    m_emitter = element["emitter"];
  } else {
    m_source = element["source"];
    m_gate = element["gate"];
    m_drain = element["drain"];
  }
}

VectorXd Hg::operator()(VectorXd &xk) {
  VectorXd xk_(xk.size() + 1);
  VectorXd zero(1);
  xk_ << zero, xk;
  ...
  if (m_elementType == "D") {
    ...
    double v_b = xk_(m_base);
    double v_c = xk_(m_collector);
    double v_e = xk_(m_emitter);
    ...
    double v_d = xk_(m_drain);
    double v_s = xk_(m_source);
    double v_g = xk_(m_gate);
...
getJacobianAndRHS(std::vector<Element> &nonlinearElements, int numNodes,
                  int numGroup2, VectorXd xk) {
  ...
  for (auto element : nonlinearElements) {
    ...
    if (elementName[0] == 'D') {
      int positiveNode = element["positiveNode"];
      int negativeNode = element["negativeNode"];
      double v_pos = xk_(positiveNode);
      double v_neg = xk_(negativeNode);
      ...
      int base = element["base"];
      int collector = element["collector"];
      int emitter = element["emitter"];
      double v_b = xk_(base);
      double v_c = xk_(collector);
      double v_e = xk_(emitter);
      ...
      int drain = element["drain"];
      int source = element["source"];
      int gate = element["gate"];
      double v_d = xk_(drain);
      double v_s = xk_(source);
      double v_g = xk_(gate);
...
bool isTerminated(MatrixXd &G, std::vector<Hg> &hg, VectorXd &s, VectorXd &xk1,
                  VectorXd &xk, VectorXd &x0) {
  VectorXd fx = G * xk1 - s;
  for (auto Hg : hg) {
    fx += Hg(xk1);
  }
  return fx.norm() <= TAU_RELATIVE * x0.norm() + TAU_ABSOLUTE &&
         (xk1 - xk).norm() <= TAU_RELATIVE * x0.norm() + TAU_ABSOLUTE;
}

double sign(double x) { return x < 0 ? -1 : 1; }

VectorXd dampStep(VectorXd delta) {
  VectorXd delta_damped(delta.size());
  for (int i = 0; i < delta.size(); i++) {
    delta_damped(i) =
        ETA / K_DAMP * sign(delta(i)) * log(1 + K_DAMP * abs(delta(i)));
  }
  return delta_damped;
}
...
VectorXd newtonMethod(std::vector<Element> &nonlinearElements, int numNodes,
  ...
  while (!isTerminated(G, hg, s, xk1, xk, xk0)) {
    auto [J, rhs] =
        getJacobianAndRHS(nonlinearElements, numNodes, numGroup2, xk1);
    xk = xk1;
    J += G;
    rhs += s;
    VectorXd xk1 = solveLinearCircuit(J, rhs);
  }
  return xk1;
}
...
VectorXd solveNonlinearCircuit(std::list<Element> &elements,
  ...
  Function f(nonlinearElements, hg, G, s, numNodes, numGroup2);
  nlopt::opt opt(nlopt::LN_COBYLA, numNodes + numGroup2);
  opt.set_min_objective(Function::wrap, &f);
  opt.set_xtol_rel(1e-4);
  std::vector<double> x(numNodes + numGroup2);
  double minf;
  try {
  ...
  VectorXd xk(numNodes + numGroup2);
  for (int i = 0; i < xk.size(); i++)
    xk(i) = x[i];
  return xk;
}

double Function::operator()(const std::vector<double> &x,
                            std::vector<double> &grad) {
  VectorXd xv(m_size);
  for (int i = 0; i < m_size; i++)
    xv(i) = x[i];
  VectorXd fx = m_G * xv - m_s;
  for (auto Hg : m_Hg) {
    fx += Hg(xv);
  }
  return fx.norm();
...
DCSweep(std::list<Element> &elements, std::vector<Element> &nonlinearElements,
  ...
  std::vector<double> inputRange, outputRange;
  auto inputElement = std::find_if(
      elements.begin(), elements.end(), [&inputNode](auto &element) {
        return element["positiveNode"] == inputNode &&
               INDEPENDENT_SOURCES.count(element.getName()[0]) == 1;
      });
  inputNode--;
  for (int i = 0; i < (end - start) / STEP + 1; i++) {
    inputElement->setValue(i * STEP);
    VectorXd x =
        solveNonlinearCircuit(elements, nonlinearElements, numNodes, numGroup2);
    inputRange.push_back(i * STEP);
    outputRange.push_back(x(inputNode));
  }
